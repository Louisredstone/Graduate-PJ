...
logpath=selfpath+"/LeNet_FPGA.log"
...
def checkRunning():
    return os.path.exists(os.path.join(cachepath,"LeNet_FPGA.running"))
...
def checkStop():
    filename=os.path.join(cachepath,"LeNet_FPGA.stopnow")
    if os.path.exists(filename):
        try:
            os.remove(filename)
            os.remove(os.path.join(cachepath,"LeNet_FPGA.running"))
        except:
            pass
        return True
    return False
...
def checkSave():#一次性标记文档检查: save
    return checkAndRemove(os.path.join(cachepath,"LeNet_FPGA.save"))

def checkLoad():#一次性标记文档检查: load
    return checkAndRemove(os.path.join(cachepath,"LeNet_FPGA.load"))

def checkLaunchTraining():#一次性标记文档检查: launch_training
    return checkAndRemove(os.path.join(cachepath,"LeNet_FPGA.launch_training"))

def checkLaunchRecog():#一次性标记文件：launch_recog
    return checkAndRemove(os.path.join(cachepath,"LeNet_FPGA.launch_recog"))

# 检查是否有其他同类运行程序的标注文件存在，以确保同一时间只有一个LeNet进程。
if checkRunning():
    exit()
open(os.path.join(cachepath,"LeNet_FPGA.running"),"w").close()#创建“正在运行”的标记文件
...
while True:
    if state == "idle":
        ...
        if checkLoad():
            ...
            continue
        if checkSave():
            savepath=selfpath+'/LeNet_FPGA.pth'
            try:
        ...
        if checkLaunchRecog():
            log("recognizing...")
            imgpath=os.path.join(cachepath,"LeNet_FPGA_recog.png")
            ...
            if np.mean(arr)>0.5:#当背景为白色时转换为黑色背景。否则识别常失效，显示为8
                arr=1-arr
            # log("debug2")
            saveImg(arr,os.path.join(cachepath,"LeNet_FPGA_recog_.png"))
            # log("debug3")
        ...
        open(os.path.join(cachepath,"LeNet_FPGA.training"),"w").close()
        for epoch in range(EPOCH):
            ...
            for i, data in enumerate(trainloader):
                ...
                if i % 100 == 99:
                    ...
                    saveImg(images_in_cpu.numpy()[0][0],os.path.join(cachepath,"FPGA_training.jpg"))
            # 每跑完一次epoch测试一下准确率
            with torch.no_grad():
                log('testing')
                correct = 0
                ...
                for data in testloader:
                    ...
                    if flag:
                        ...
                        flag=False
                log('第%d个epoch的识别准确率为：%d%%' % (epoch + 1, (100 * correct / total)))
        checkAndRemove(os.path.join(cachepath,"LeNet_FPGA.training"))

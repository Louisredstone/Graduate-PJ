...
logpath=selfpath+"/LeNet_GPU.log"
...
def checkRunning():
    return os.path.exists(os.path.join(cachepath,"LeNet_GPU.running"))
...
def checkStop():
    filename=os.path.join(cachepath,"LeNet_GPU.stopnow")
    if os.path.exists(filename):
        try:
            os.remove(filename)
            os.remove(os.path.join(cachepath,"LeNet_GPU.running"))
        except:
            pass
        return True
    return False
...
def checkSave():#一次性标记文档检查: save
    return checkAndRemove(os.path.join(cachepath,"LeNet_GPU.save"))

def checkLoad():#一次性标记文档检查: load
    return checkAndRemove(os.path.join(cachepath,"LeNet_GPU.load"))

def checkLaunchTraining():#一次性标记文档检查: launch_training
    return checkAndRemove(os.path.join(cachepath,"LeNet_GPU.launch_training"))

def checkLaunchRecog():#一次性标记文件：launch_recog
    return checkAndRemove(os.path.join(cachepath,"LeNet_GPU.launch_recog"))

# 检查是否有其他同类运行程序的标注文件存在，以确保同一时间只有一个LeNetCPU进程。（CPU进程可以和GPU进程共存）
if checkRunning():
    exit()
open(os.path.join(cachepath,"LeNet_GPU.running"),"w").close()#创建“正在运行”的标记文件
...
while True:
    if state == "idle":
        ...
        if checkLoad():
            ...
            continue
        if checkSave():
            savepath=selfpath+'/LeNet_GPU.pth'
            try:
        ...
        if checkLaunchRecog():
            log("recognizing...")
            imgpath=os.path.join(cachepath,"LeNet_GPU_recog.png")
            ...
            if np.mean(arr)>0.5:#当背景为白色时转换为黑色背景。否则识别常失效，显示为8
                arr=1-arr
            # log("debug2")
            saveImg(arr,os.path.join(cachepath,"LeNet_GPU_recog_.png"))
            # log("debug3")
        ...
        open(os.path.join(cachepath,"LeNet_GPU.training"),"w").close()
        for epoch in range(EPOCH):
            ...
            for i, data in enumerate(trainloader):
                ...
                if i % 100 == 99:
                    ...
                    saveImg(images_in_cpu.numpy()[0][0],os.path.join(cachepath,"GPU_training.jpg"))
            # 每跑完一次epoch测试一下准确率
            with torch.no_grad():
                log('testing')
                correct = 0
        ...
        checkAndRemove(os.path.join(cachepath,"LeNet_GPU.training"))
